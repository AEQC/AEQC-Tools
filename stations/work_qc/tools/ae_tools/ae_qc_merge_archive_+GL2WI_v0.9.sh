...
do
 ...
 then
  ...
  then
   ### MSREPACK BUG OU PRB CODAGE MINISEED NMXPTOOL
   ...
   ### PATCH TEMPORAIRE 22/06/2012
   echo "Stop process file block size is not 512 bytes !"
   kill $$
  fi
  ...
  then
   ...
   ##mv -f file_temp2  $file_name3
   ### PATCH TEMPORAIRE 22/06/2012
   echo "Stop process file  is code with STEIM2, STEIM1 required !"
   kill $$
  fi
 fi #=> if [ -s $file_name3 ]
 ####################################################################
 ...
 ###########################################
 destination_exist=0
 if [ $file3 == 1 -o $file4 == 1 ]
 then
  destination_exist=1
 fi
 echo "destination_exist="$destination_exist
 ###########################################
 if [ $reseau == GL -a $destination_exist == 1 ]
 then
  if [ $station_name == DHS -o $station_name == DSD -o $station_name == CBE -o $station_name == MAGL ]
  then 
   dir_data_station_11=$dir_data_base_1'/'$year1'/'WI'/'$station_name'/'$canal'.D/'
   echo 'dir_data_station_11 = '$dir_data_station_11
   if [ ! -d $dir_data_station_11 ]
   then
    echo "Error $dir_data_station_11 doesn't exist, try to create it !"
    mkdir -p $dir_data_base_1/$year1
    mk=$?
    mkdir -p $dir_data_base_1/$year1/WI
    mk=$?
    mkdir -p $dir_data_base_1/$year1/WI/$station_name
    mk=$?
    mkdir -p $dir_data_base_1/$year1/WI/$station_name/$canal.D
    mk=$?
    echo "mk="$mk
    if [ $mk != 0 ]
    then
     echo "can't create required dir, stop process !"
     kill $$
    fi
   fi #=> if [ ! -d $dir_data_station_11 ]
   ####################################################################
   file_name11=WI'.'$station'.'$location'.'$canal'.D.'$year_file1'.'$day_file1
   echo 'file_name11 = '$file_name11
   ####################################################################
   file_name33=$dir_data_station_11$file_name11
   echo "** file_name33 = "$file_name33
   ####################################################################
   if [ -s $file_name33 ]
   then
    echo "#################### GET LOCAL SIZE_BLOCK-ENCODING-QUALITY_FLAG $file_name33 "
    $dir_bin_base/msview -p $file_name33 |  head -n 14 > output_ms_view.txt  
    echo "*********output_ms_view.txt local $file_name33 *************"
    cat output_ms_view.txt
    echo "****************************************"
    local_file1_block_size=$(cat output_ms_view.txt | grep record | awk '{print $3}' )
    echo "local_file1_block_size="$local_file1_block_size
    ...
    echo "local_file1_quality_flag="$local_file1_quality_flag    
    echo "conversion local file1 to D quality flag."
    $dir_bin_base/dataselect -Q D -o file_temp  $file_name33
    mv -f file_temp $file_name33
    if [ $local_file1_block_size  != 512 ]
    then
     ### MSREPACK BUG OU PRB CODAGE MINISEED NMXPTOOL
     #echo "conversion local file1 to 512 block size."   
     #$dir_bin_base/msrepack -R 512 -o file_temp $file_name33  >& /dev/null
     #$dir_bin_base/qmerge -b 512 file_temp > file_temp2
     #mv -f file_temp2 $file_name33 
     ### PATCH TEMPORAIRE 22/06/2012
     echo "Stop process file block size is not 512 bytes !"
     kill $$
    fi
    ...
    then
     ### MSREPACK BUG OU PRB CODAGE MINISEED NMXPTOOL
     #echo "conversion local file1 to STEIM 2 encoding."
     #$dir_bin_base/msrepack -E 11 -o file_temp $file_name33  >& /dev/null
     #kill $$
     #$dir_bin_base/qmerge -b 512 file_temp > file_temp2
     #mv -f file_temp2  $file_name33 
     ### PATCH TEMPORAIRE 22/06/2012
    ...
    $dir_bin_base/qmerge -b 512 -N WI test3 > test33
    $dir_bin_base/qmerge -b 512 test33 $file_name33 > test333
    mv -f test333 $file_name33
   else #=> if [ -s $file_name33 ]
    $dir_bin_base/qmerge -b 512 -N WI test3 > $file_name33
   fi #=> if [ -s $file_name33 ]
  fi #=> if [ $station_name == DHS -o $station_name == DSD ]
 fi #=> if [ $reseau == GL ] 
 #kill $$
 ###########################################
